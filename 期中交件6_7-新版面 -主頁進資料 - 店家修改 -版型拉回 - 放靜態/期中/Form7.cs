...
namespace 期中
{
    public partial class Form7 : Form
    {
        ...
        {
            // TODO: 這行程式碼會將資料載入 'mydbDataSet4.店家' 資料表。您可以視需要進行移動或移除。
            this.店家TableAdapter1.Fill(this.mydbDataSet4.店家);
            // TODO: 這行程式碼會將資料載入 'mydbDataSet4.商品2' 資料表。您可以視需要進行移動或移除。
            this.商品2TableAdapter.Fill(this.mydbDataSet4.商品2);
            // TODO: 這行程式碼會將資料載入 'mydbDataSet4.店家5' 資料表。您可以視需要進行移動或移除。
            this.店家5TableAdapter.Fill(this.mydbDataSet4.店家5);

            form5.Visible = true;



            // TODO: 這行程式碼會將資料載入 'mydbDataSet3.商品' 資料表。您可以視需要進行移動或移除。
            this.商品TableAdapter.Fill(this.mydbDataSet3.商品);
            // TODO: 這行程式碼會將資料載入 'mydbDataSet3.店家' 資料表。您可以視需要進行移動或移除。
            this.店家TableAdapter.Fill(this.mydbDataSet3.店家);
        ...
        private void btn更新_Click(object sender, EventArgs e)
        {
            ...
            if (intID > 0)
            {
                if ((txt姓名.Text != "") && (txtEmail.Text != ""))
                {
                    ...
                    con.Open();
                    //sql 更新語法 下指令
                    string strSQL = "update 學生 set stName=@NewName, 電話=@NewPhone, 地址=@NewAddress, email=@NewEmail, 生日=@NewBirth, stAuthority=@NewAuthority,班級=@NewClass, stPassword=@NewPassword where id=@SearchID;";
                    SqlCommand cmd = new SqlCommand(strSQL, con);
                    //補參數
        ...
        private void lbox搜尋結果_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                int intID = SearchIDs[lbox搜尋結果.SelectedIndex]; // 結果ID串列[所選索引]           item[0]=id[0] 第0筆的ID等於ID串列第0筆 對應

                //SOP
                SqlConnection con = new SqlConnection(myDBConnectionString);
                con.Open();
                string strSQL = "select * from 學生 where id = @SearchID";  //指定尋找id
                SqlCommand cmd = new SqlCommand(strSQL, con);
                cmd.Parameters.AddWithValue("@SearchID", intID);
                SqlDataReader reader = cmd.ExecuteReader();
                //讀入sop //字串給入欄位
                if (reader.Read())                   //Slected單筆觸發 Read讀到給單筆
                {
                    txtID.Text = string.Format("{0}", reader["id"]);
                    txt姓名.Text = string.Format("{0}", reader["stName"]);
                    txt電話.Text = string.Format("{0}", reader["電話"]);
        ...
        private void fillByToolStripButton_Click(object sender, EventArgs e)
        {
            try
            {
                this.商品TableAdapter.FillBy(this.mydbDataSet3.商品);
            }
        ...
        private void fillBy1ToolStripButton_Click(object sender, EventArgs e)
        {
            try
            {
                this.商品TableAdapter.FillBy1(this.mydbDataSet3.商品);
            }
            catch (System.Exception ex)
            {
                System.Windows.Forms.MessageBox.Show(ex.Message);
        ...
        private void 店家DataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
           
        }

        private void 店家BindingSource3_CurrentChanged(object sender, EventArgs e)
        {

        }

        private void 商品BindingSource_CurrentChanged(object sender, EventArgs e)
        {

        }

        private void 商品2BindingSource1_CurrentChanged(object sender, EventArgs e)
        {
        ...
        private void btn上一筆_Click(object sender, EventArgs e)
        {
            ...
            商品2BindingSource.MovePrevious();
        }

        private void btn下一筆_Click(object sender, EventArgs e)
        {
            商品2BindingSource.MoveNext();

        }

        private void btn最後筆_Click(object sender, EventArgs e)
        {
            商品2BindingSource.MoveLast();

        }

        private void btn第一筆_Click(object sender, EventArgs e)
        {
            商品2BindingSource.MoveFirst();
        ...
        private void btn儲存商品資料_Click(object sender, EventArgs e)
        {
            DialogResult R = MessageBox.Show("確認儲存??", "儲存確認", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
            // MessageBox.Show("test", "test", MessageBoxButtons.OK,MessageBoxIcon.Question);

            if (R == DialogResult.Yes)//user點擊 MessageBox回傳值儲存在R變數
            {
                商品2BindingSource.EndEdit();//結束編輯模式  不管在甚麼狀態(ex:輸入游標)都結束掉 UI內的內容都儲存異動  資料都存入BindingSource的物件
                tableAdapterManager3.UpdateAll(this.mydbDataSet4);//異動寫入資料庫 
                string pic = "9.png";
                商品照片.Image = Image.FromFile(@"image\" + pic);
            }
            else
            {
                //取消 無動作
        ...
        private void btn上傳照片_Click(object sender, EventArgs e)
        {
            ...
            pictureBox1.Image = Image.FromFile(@"image\" + pic);
        }

        private void 商品2DataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            label28.Text =txt餐點名稱.Text;
        }
        ...
        //    e.Graphics.DrawString(listBox2學員.Items[e.Index].ToString(), e.Font,
